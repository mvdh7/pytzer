...
def _bC_plot(ax, xtype, tots, mols, ele, ions, tempK, pres, prmlib_base, varout):
    ifuncs = meta.getifuncs("bC", ions)
    fvar = full((len(ifuncs), len(tempK)), nan)
    fvarfuncs = {
        "acf_anion": (
            lambda mols, ions, tempK, pres, prmlib: model.acfs(
                mols, ions, tempK, pres, prmlib=prmlib
            )[1],
    ...
    for i, ifunc in enumerate(ifuncs):
        ...
        fvar[i] = fvarfuncs[varout][0](mols, ions, tempK, pres, prmlib=prmlib_base)
    xtypes = {
...
def bC_pres(
    ...
    fvar = _bC_plot(ax, "pres", tots, mols, ele, ions, tempK, pres, prmlib_base, varout)
    return fvar
...
def bC_tempK(
    ...
    fvar = _bC_plot(
        ax, "tempK", tots, mols, ele, ions, tempK, pres, prmlib_base, varout
    )
    return fvar
...
def bC_tot(
    ...
    fvar = _bC_plot(ax, "tot", tots, mols, ele, ions, tempK, pres, prmlib_base, varout)
    return fvar
