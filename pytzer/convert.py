...
def solvent_to_solution(molalities, pks):
    """Converts concentrations and equilibrium constants (pKs) from molality (mol/kg-solvent) to molinity (mol/kg-solution).
    ...
    molinities = OrderedDict(
                    (key, molalities[key] * h2o)
                    for key in molalities.keys()
    )
    ...
    pks_out = OrderedDict(
                    (key, - np.log10(ks_out[key]))
                    for key in ks_out.keys()
    )
