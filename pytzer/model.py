...
def Gibbs_map(
    ...
    def add_cc(i_xy):
        ix, iy = i_xy
        return m_cats[ix] * m_cats[iy] * (cc[ix][iy] +
            etheta(
            Aphi, I, z_cats[ix], z_cats[iy], func_J=func_J
        ))


    def add_aa(i_xy):
        ix, iy = i_xy
        return m_anis[ix] * m_anis[iy] * (aa[ix][iy] +
            etheta(
            Aphi, I, z_anis[ix], z_anis[iy], func_J=func_J
        ))
    ...
    i_cca = np.array(list(itertools.product(range(n_cats), range(n_cats), range(n_anis))))
    i_caa = np.array(list(itertools.product(range(n_cats), range(n_anis), range(n_anis))))
    ...
    i_nca = np.array(list(itertools.product(range(n_neus), range(n_cats), range(n_anis))))
...
def log_activity_coefficients_map(m_cats, m_anis, m_neus, z_cats, z_anis, **parameters):
    """Calculate the natural log of the activity coefficient of all solutes."""
    log_acf_cats = jax.grad(Gibbs_map, 0)(
        m_cats, m_anis, m_neus, z_cats, z_anis, **parameters
    )
    log_acf_anis = jax.grad(Gibbs_map, 1)(
        m_cats, m_anis, m_neus, z_cats, z_anis, **parameters
    )
    log_acf_neus = jax.grad(Gibbs_map, 2)(
        m_cats, m_anis, m_neus, z_cats, z_anis, **parameters
    )
    return log_acf_cats, log_acf_anis, log_acf_neus
