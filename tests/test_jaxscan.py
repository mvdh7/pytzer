...
cats = np.vstack([1., 2, 3, 4, 5])
anis = np.array([[5., 2, 6]])
ca = np.array([[3., 1, 4, 7, 5],
               [3, 2, 6, 8, 4],
               [1, 4, 3, 6, 8]])
...
@jit
def test_matmul():
    anisanis = np.array([(np.transpose(anis) @ anis)
            [np.triu_indices(len(anis[0]), k=1)]])
    catscats = np.array([(np.transpose(cats) @ cats)
            [np.triu_indices(len(cats[0]), k=1)]])
