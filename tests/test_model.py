...
# def test_model_loop_functions():
#     """Do all the model loop functions return floats?"""
#     Gibbs_nRT = pz.model.Gibbs_nRT_loop(*args, **params)
#     assert isinstance(Gibbs_nRT.item(), float)
#     log_activity_water = pz.model.log_activity_water_loop(*args, **params)
#     assert isinstance(log_activity_water.item(), float)
#     activity_water = pz.model.activity_water_loop(*args, **params)
#     assert isinstance(activity_water.item(), float)
#     osmotic_coefficient = pz.model.osmotic_coefficient_loop(*args, **params)
#     assert isinstance(osmotic_coefficient.item(), float)
#     log_activity_coefficients = pz.model.log_activity_coefficients_loop(*args, **params)
#     for x in log_activity_coefficients:
#         assert isinstance(x[0].item(), float)
#     activity_coefficients = pz.model.activity_coefficients_loop(*args, **params)
