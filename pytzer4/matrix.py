...
def etheta(Aosm, I, zs):
    ...
    x00 = xi(Aosm, I, zs)
    x11 = xj(Aosm, I, zs)
    return (transpose(zs) @ zs) * (jfunc(x01) - (jfunc(x00) + jfunc(x11)) / 2) / (4 * I)
...
def Gex_nRT(mols, allmxs):
    ...
    if I == 0:
        Gex_nRT = (
            mols @ lambdamx @ transpose(mols)
            + neus @ zetamx @ transpose(catsanis)
            + neus**3 @ mumx
        )[0]
    else:
        Z = Zstr(mols, zs)
        zcats = array([zs[zs > 0]])
        zanis = array([zs[zs < 0]])
        catscats = array([(transpose(cats) @ cats)[triu_indices(len(cats[0]), k=1)]])
        anisanis = array([(transpose(anis) @ anis)[triu_indices(len(anis[0]), k=1)]])
        Gex_nRT = (
            fG(Aosm, I)
            + mols
            @ (
...
def lnaw(mols, allmxs):
    ...
    ww = 1.0
    return (
        egrad(lambda ww: ww * Gex_nRT(mols / ww, allmxs))(ww) - np_sum(mols, axis=1)
    ) * Mw
...
def osm(mols, allmxs):
    ...
    ww = 1.0
    return 1.0 - egrad(lambda ww: ww * Gex_nRT(mols / ww, allmxs))(ww) / np_sum(mols)
    ...
    for IX, ionx in enumerate(ions):
        for IY, iony in enumerate(ions):
            if ionx in cations and iony in anions:
                ...
                (ionx in cations and iony in cations)
                or (ionx in anions and iony in anions)
            ) and ionx != iony:
                iset = [ionx, iony]
                iset.sort()
                iset = "-".join(iset)
    ...
    for N, neutral in enumerate(neutrals):
        ...
        for C, cation in enumerate(cations):
            for A, anion in enumerate(anions):
                ...
                zetamx[N, C * size(anions) + A] = prmlib.zeta[iset3](tempK, pres)[0]
    return (
        zs,
        Aosm,
        b0mx,
