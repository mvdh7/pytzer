...
def gettots(filename, delimiter=",", skip_top=0):
    ...
    tots = array([tot for i, tot in enumerate(data) if "t_" in head[i]])
    mols = array([mol for i, mol in enumerate(data) if "t_" not in head[i]])
...
def saveall(filename, mols, ions, tempK, pres, osm, aw, acfs):
    ...
    savetxt(
        ...
        header=",".join(
            concatenate(
                (["tempK", "pres"], ions, ["osm", "aw"], ["g" + ion for ion in ions])
            )
        ),
        comments="",
    )
...
def salinity2mols(salinity, MgOH=False):
    ...
    """
    if MgOH:
        mols = array([0.44516, 0.01077, 0.01058, 0.56912]) * salinity / 43.189
        ions = array(["Na", "Ca", "K", "Cl"])
        tots = array([0.02926, 0.05518]) * salinity / 43.189
        eles = array(["t_HSO4", "t_Mg"])
    else:
        mols = array([0.44516, 0.05518, 0.01077, 0.01058, 0.56912]) * salinity / 43.189
