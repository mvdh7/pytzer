...
def blackbox(filename, prmlib=libraries.MIAMI, savefile=True):
    ...
    if savefile:
        ...
        io.saveall(filestem + "_py.csv", mols, ions, tempK, pres, osm, aw, acfs)
    print("Finished!")
...
def blackbox_equilibrate(filename, prmlib=libraries.MIAMI, savefile=True):
    ...
    allmols, allions, eqstates = equilibrate.solveloop(
        eqstate_guess, tots, fixmols, eles, fixions, tempK, pres, prmlib=prmlib
    )
    # Separate out zero ionic strengths
    ...
    if savefile:
        filestem = filename.replace(".csv", "")
        io.saveall(filestem + "_py.csv", allmols, allions, tempK, pres, osm, aw, acfs)
    print("Finished!")
    return (allmols, allions, tempK, pres, prmlib, Gex_nRT, osm, aw, acfs, eqstates)
