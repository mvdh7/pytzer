...
PMcats = ["", "Sr", "Na", "K", "Mg", "Ca", "H", "Li", "Rb", "Cs", "NH4", "Ba", "Mn"]
mjCaMax = 5
T = np.pi * 100
P = 10.10325
Tcc = np.zeros((13, 13))
...
PManis = ["", "Cl", "SO4", "CO3", "HCO3", "Br", "F", "BOH4", "HSO4", "HS", "OH"]
mjAnMax = 7
Taa = np.zeros((13, 13))
Taa[1, 2] = 0.07
Taa[1, 3] = -0.053
...
Taa[mjAnMax + 1, 2] = 0
Taa[mjAnMax + 3, 1] = -0.05 + 0.0003125 * (T - 298.15) - 0.000008362 * (T - 298.15) ** 2
...
for a0, ani0 in enumerate(PManis):
    for a1, ani1 in enumerate(PManis):
        ...
        for c, cat in enumerate(PMcats):
            ...
            if caastr in prmlib.psi.keys() and caastr not in Paac_compare.keys():
...
for c0, cat0 in enumerate(PMcats):
    for c1, cat1 in enumerate(PMcats):
        ...
        for a, ani in enumerate(PManis):
            ...
            if ccastr in prmlib.psi.keys() and ccastr not in Pcca_compare.keys():
