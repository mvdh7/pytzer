...
def Gtest(mols, allmxs):
    ...
    ethetamx = (pz.matrix.etheta(Aosm, I, zs_cats) +
        pz.matrix.etheta(Aosm, I, zs_anis))
    Gex_nRT0 = (mols @ thetamx @ transpose(mols) +
        cats @ pz.matrix.etheta(Aosm, I, zcats) @ transpose(cats) +
        anis @ pz.matrix.etheta(Aosm, I, zanis) @ transpose(anis))[0]
    Gex_nRT1 = (mols @ (thetamx + ethetamx) @ transpose(mols))[0]
    return (Gex_nRT0, Gex_nRT1, ethetamx, thetamx, I,
        cats @ pz.matrix.etheta(Aosm, I, zcats) @ transpose(cats),
...
(Gt0, Gt1, Gethetamx, Gthetamx, I, Gbitcat, Gbitmol,
    Gwtf1, Gwtf2, Giz, Gizc) = Gtest(mols_mx, allmxs)
from autograd import elementwise_grad as egrad
Gacf0 = np.exp(egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[0])(mols_mx, allmxs))
Gacf1 = np.exp(egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[1])(mols_mx, allmxs))

Gbitgradcat = egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[5])(mols_mx, allmxs)
Gbitgradmol = egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[6])(mols_mx, allmxs)
Ggradwtf1 = egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[7])(mols_mx, allmxs)
Ggradwtf2 = egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[8])(mols_mx, allmxs)
Ggradiz = egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[9])(mols_mx, allmxs)
Ggradizc = egrad(lambda mols_mx, allmxs:
    Gtest(mols_mx, allmxs)[10])(mols_mx, allmxs)    
    
Jtest = (pz.matrix.jfunc(6*Aosm*np.sqrt(Istr_mx) 
    * np.abs(transpose(zs_mx) @ zs_mx)))
